...
export default function App() {
  ...
  const Header = ({ eyebrow, title, right, big }) => (
    <div className="head">
      <div className="nf-wrap" style={{ paddingTop: 17, paddingBottom: big ? 24 : 17, position: "relative", zIndex: 1 }}>
        ...
        <div className="nf-brand-lockup" style={{ marginTop: 13 }}>
          <div className={`nf-brand-mark ${big ? "" : "small"}`}>
            <Logo size={big ? 60 : 38} />
          </div>
          <div style={{ minWidth: 0 }}>
            {eyebrow && <div className="eyebrow" style={{ color: c.gold, marginBottom: 3 }}>{eyebrow}</div>}
            <h1 className="display nf-brand-title" style={{ fontSize: big ? 46 : 31 }}>
              {title || <>NECTAR<span style={{ color: c.gold }}>FUSIONS</span></>}
            </h1>
            {big && (
              <div className="nf-brand-tagline">
                <span className="script" style={{ color: c.gold, fontSize: 21 }}>nature&rsquo;s happiness</span>
                <span style={{ color: "#8E7C64" }}>|</span>
                <span className="eyebrow" style={{ color: "#E9DCC8" }}>honey infused</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="nf-wrap" style={{ paddingTop: 22 }}>
        ...
        {pickSize === B.size && (
          <div className="card" style={{
            ...
            borderColor: "#1475AE",
            boxShadow: "0 10px 24px rgba(36,160,237,.20)",
            textAlign: "center"
          }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              <div style={{
                ...
                {price.jars % B.count === 0 && price.bundles === 0 && (
                  <div style={{ marginTop: 2 }}>Regular and Spun both count.</div>
                )}

                {price.jars % B.count > 0 && (
                  <div style={{ marginTop: 2 }}>
                    <strong>{B.count - (price.jars % B.count)} more</strong> 4 oz to complete a bundle.
                  </div>
                )}

                {price.jars % B.count === 0 && price.bundles > 0 && (
                  <div style={{ marginTop: 2, color: "#FFF4B8" }}>
