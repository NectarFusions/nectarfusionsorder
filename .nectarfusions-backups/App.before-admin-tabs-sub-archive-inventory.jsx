...
function Admin({ cat, reload, Header, onExit, onSignOut }) {
  ...
  const shown = orderPool.filter((o) => !q ||
    ...
    <div className="nf"><style>{CSS}</style>
      ...
      <div className="nf-wrap" style={{ paddingTop: 22 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
          <button
            className={`btn ${orderView === "active" ? "on" : ""}`}
          ...
          <button
            className={`btn ${orderView === "archived" ? "on" : ""}`}
            style={{ padding: 10, fontSize: 13 }}
              ...
              <div key={o.id} className="card" style={{ padding: 13, marginBottom: 8, opacity: cx ? .5 : 1,
                ...
                {orderView === "active" ? (
                  <div style={{ marginTop: 10 }}>
                    ...
                    <button
                      className="btn ghost"
                      style={{ width: "100%", padding: "9px 12px", marginTop: 7, fontSize: 12.5 }}
                      onClick={() =>
                        confirm(`Archive order #${o.order_no}?\n\nIt will move out of the active list but remain safely stored.`) &&
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: o.paid ? "1fr" : "1fr 1fr", gap: 7, marginTop: 10 }}>
                    <button
                      ...
                      onClick={() => guard(() => api.restoreOrder(o.id))}
                    >
                      Restore
                    </button>
                    ...
                    {!o.paid && (
                      <button
                        ...
                        onClick={() =>
                          confirm(
                            `Permanently delete order #${o.order_no}?\n\nThis removes the order and its jar list forever. This cannot be undone.`
                    ...
                    {o.paid && (
                      <div style={{ gridColumn: "1 / -1", fontSize: 11.5, lineHeight: 1.5, color: c.brown, textAlign: "center" }}>
                        Paid orders stay archived and cannot be permanently deleted.
                      </div>
                    )}
                  </div>
                )}
